...
class StoreServiceCatalogItemRequest extends FormRequest
{
    ...
    {
        return $this->user()?->can('create', ServiceCatalogItem::class) ?? false;
    }
    ...
    public function rules(): array
    {
        return [
            'name' => [
                ...
                'string',
                'max:150',
                Rule::unique(ServiceCatalogItem::class, 'name')->where(
                    fn ($query) => $query->where('category', $this->input('category')),
                ),
            ],
